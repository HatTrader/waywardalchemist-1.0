...
public class BoilingBrewerScreen extends HandledScreen<BoilingBrewerScreenHandler> {
    ...
    public BoilingBrewerScreen(BoilingBrewerScreenHandler handler, PlayerInventory inventory, Text title) {
    ...
    protected void drawBackground(DrawContext context, float deltaTicks, int mouseX, int mouseY) {
        ...
        int k = ((BoilingBrewerScreenHandler)this.handler).getFuel();
        int l = MathHelper.clamp((18 * k + 20 - 1) / 20, 0, 18);
        if (l > 0) {
            context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, FUEL_LENGTH_TEXTURE, 18, 4, 0, 0, i + 60, j + 44, l, 4);
        }

        int m = ((BoilingBrewerScreenHandler)this.handler).getBrewTime();
        if (m > 0) {
